<div class="container mx-auto p-4 bg-white dark:bg-gray-900 min-h-screen">
    <h2 class="text-4xl font-bold mb-4 text-gray-800 dark:text-gray-100">Détails des réservations</h2>

    @foreach($reservations as $reservation)
    <div class="row flex flex-col bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden mb-4">
        <div class="p-4 flex flex-col md:flex-row">
            <!-- Affichage de l'image de la propriété -->
            @if($reservation->property->images->isNotEmpty())
            <img src="{{ asset('storage/' . $reservation->property->images->first()->image_path) }}" alt="Image de la propriété" class="object-cover w-full rounded-t-lg h-96 md:h-auto md:w-48 md:rounded-none md:rounded-s-lg bg-gray-100 dark:bg-gray-700">
            @else
            <img src="{{ asset('images/default-property.jpg') }}" alt="Image par défaut" class="object-cover w-full rounded-t-lg h-96 md:h-auto md:w-48 md:rounded-none md:rounded-s-lg bg-gray-100 dark:bg-gray-700">
            @endif
            <div class="flex flex-col justify-between flex-grow p-4">
                <div>
                    <h3 class="text-lg text-gray-800 dark:text-gray-100">{{ $reservation->property->name ?? 'Nom non disponible' }}</h3>
                    <p class="text-gray-500 dark:text-gray-300">{{ $reservation->start_date }} - {{ $reservation->end_date }}</p>
                    <p class="text-gray-600 dark:text-gray-200">{{ $reservation->total_price }} FrCFA</p>
                    <p class="text-gray-400 dark:text-gray-400">Soumis le : {{ $reservation->created_at }}</p>
                </div>
                <div class="flex justify-end mt-4">
                    @if($this->hasReview($reservation->property->id))
                    <button wire:click="openEditReviewModal({{ $reservation->property->id }})" class="bg-yellow-500 hover:bg-yellow-600 text-white py-1 px-2 rounded transition">Modifier l'avis</button>
                    @else
                    <button wire:click="openReviewModal({{ $reservation->id }})" class="bg-blue-500 hover:bg-blue-600 text-white py-1 px-2 rounded transition">Laissez un avis</button>
                    @endif
                </div>
            </div>
        </div>
